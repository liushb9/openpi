...
class SftDataset(Dataset):
    def __init__(self, config, processor,accelerator, model):
        ...
        self.action_tokenizer = ActionTokenizer(self.tokenizer, bins = self.config.bins)
    ...
    def center_crop(self, img_path, crop_size=384):
    ...
    def process_image(self,image_paths):
        ...
        # images = [self.center_crop(image_path).resize((384, 384), PIL.Image.Resampling.LANCZOS) for image_path in image_paths]
...
def train(args: argparse.Namespace) -> None:
    ...
    for epoch in range(0, args.n_epochs):
        ...
        for batch in train_iter:
            ...
            if args.image_generation:
                ### +2 means: <image start token> 
                action_tokens = batch['input_ids'][:, -(image_embeds.shape[1]+1+args.action_dim) : -(image_embeds.shape[1]+1)].contiguous()
                action_logits = model.language_model.lm_head(hidden_states[:, -(image_embeds.shape[1]+2+args.action_dim) : -(image_embeds.shape[1]+2), :])
