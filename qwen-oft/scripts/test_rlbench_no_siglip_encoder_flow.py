...
def model_predict(args, vl_gpt, vl_chat_processor, action_tokenizer, statistic, task_description, image, state, pointcloud, pre_image_dir, step):
    ...
    with torch.inference_mode():
        ...
        noise = torch.randn(inputs_embeds.shape[0], args.action_chunk, 7, dtype=torch.bfloat16, device=device)
        samples = vl_gpt.forward_flow(inputs_embeds, noise)
        
        normalized_actions = samples[0].cpu().to(torch.float32).numpy()
