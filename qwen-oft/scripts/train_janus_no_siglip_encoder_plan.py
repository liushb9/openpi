...
class TrainingMetrics:
    ...
    def update_action(self, action_logits, action_labels, action_loss):
        self.n_step += 1
        with torch.no_grad():
            shift_action_preds = action_logits[..., :-1, :].argmax(dim=-1)
            shift_action_labels = action_labels[..., 1:]
            self.action_right += (shift_action_preds == shift_action_labels).masked_fill(shift_action_labels.eq(-100), 0).sum().item()
            self.action_total += (shift_action_labels != -100).sum().item()
            self.action_loss += action_loss.item()
...
class SftDataset(Dataset):
    def __init__(self, config, processor,accelerator, model):
        ...
        self.action_tokenizer = ActionTokenizer(self.tokenizer, bins = self.config.bins, need_to_sub=self.config.need_to_sub)
    ...
    def modify_labels_vectorized(self, labels):
        labels_modified = labels.clone()
        
        BOP_id = self.processor.BOP_id
        BOA_id = self.processor.BOA_id
        
        batch_size, seq_len = labels.shape
        positions = torch.arange(seq_len).expand(batch_size, seq_len)
        
        mask_token_1 = labels == BOP_id
        mask_token_2 = labels == BOA_id
        
        first_token_1_pos = (mask_token_1 * positions).max(dim=1).values
        first_token_2_pos = (mask_token_2 * positions).max(dim=1).values
        
        mask_1 = positions < first_token_1_pos.unsqueeze(1)
        mask_2 = positions < first_token_2_pos.unsqueeze(1)
        
        use_token_1 = mask_token_1.any(dim=1)
        final_mask = torch.where(use_token_1.unsqueeze(1), mask_1, mask_2)

        labels_modified[final_mask] = self.processor.ignore_id
    ...
    def collate_fn(self, batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        for x in batch:
            ...
            plan_label = f"<BOP>{x['language_plan']}<EOP>" if x['language_plan'] != "" else ""

            conversation = [
                {"role": "<|User|>","content": prompts},
                {"role": "<|Assistant|>", "content": f"{plan_label}<BOA>{action_tokens}<EOA>"}
            ]
        ...
        labels = self.modify_labels_vectorized(prepare_inputs.input_ids)
    ...
    for epoch in range(0, args.n_epochs):
        ...
        for batch in train_iter:
            ...
            if batch['input_pixel_values'] is not None:
                ...
                batch['input_ids'][batch['input_ids'] < 0] = 0
            ...
            loss = outputs.loss
            metric(args.image_generation, None, None, None, outputs.logits, batch['labels'], loss)
