...
class SftDataset(Dataset):
    ...
    def process_image(self,image_paths):
        ...
        images = [self.center_crop(image_path).resize((384, 384), Image.Resampling.LANCZOS) for image_path in image_paths]
    ...
    def collate_fn(self, batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        for x in batch:
            ...
            prompts = x['input_prompt'] + input_img_tokens * img_len + output_img_tokens
        ...
        for batch in train_iter:
            ...
            if batch['input_pixel_values'] is not None:
                ...
                if args.image_generation:
                    num_placeholders_per_sample = (image_embeds_input.shape[0] // args.train_bsz_per_gpu) + 1
                    image_gen_indices = [
                        ind for ii, ind in enumerate(image_gen_indices) 
                        if (ii + 1) % num_placeholders_per_sample != 0
                    ]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bins', type=int, default=42, help='Random seed')
