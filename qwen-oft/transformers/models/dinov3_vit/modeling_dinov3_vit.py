    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        Args:
            ...
            output_qkv: Whether to output QKV states
            qkv_after_rope: If True, extract QKV after applying RoPE; if False, extract before RoPE
        """
        ...
        # Extract QKV before RoPE if requested
        qkv_states = None
        if output_qkv and not qkv_after_rope:
            qkv_states = {
                'query': query_states.clone(),  # [batch_size, num_heads, patches, head_dim]
        ...
        # Extract QKV after RoPE if requested (default behavior)
        if output_qkv and qkv_after_rope:
            qkv_states = {
                'query': query_states,  # [batch_size, num_heads, patches, head_dim]
                'key': key_states,
                'value': value_states,
            }
        ...
        return attn_output, attn_weights, qkv_states
...
class DINOv3ViTLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        hidden_states, attn_weights, qkv_states = self.attention(
            hidden_states,
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        output_attentions (`bool`, *optional*, defaults to False):
            Whether to output attention weights from all attention layers.
        output_qkv (`bool`, *optional*, defaults to False):
            Whether to output query, key, value states from all attention layers.
        qkv_after_rope (`bool`, *optional*, defaults to True):
            If True, extract QKV states after applying RoPE; if False, extract before RoPE.
        """
        ...
        all_qkv_states = [] if output_qkv else None
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if output_qkv and layer_outputs[2] is not None:
                all_qkv_states.append(layer_outputs[2])

        sequence_output = self.norm(hidden_states)
        pooled_output = sequence_output[:, 0, :]

        if not return_dict:
            return (sequence_output, pooled_output, all_attentions, all_qkv_states)
        ...
        # Add qkv_states as an extra field
        if output_qkv:
            output.qkv_states = all_qkv_states
        if output_attentions:
