...
def build_mask(q_length, kv_length, action_length, device=None):
    query_positions = torch.arange(q_length, device=device).view(1, 1, q_length, 1)
...
def change_mask(attention_mask, action_length, device=None):
    if action_length > 0:
...
# # ---------ch changed----------- # 
    if 'action_length' in kwargs:
        ...
        if attention_mask is None:
            attention_mask = build_mask(query.shape[-2], key.shape[-2], action_length, device=query.device)
        else:
            attention_mask = change_mask(attention_mask, action_length, device=query.device)
