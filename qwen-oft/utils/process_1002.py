...
def folder_2_jsonl(data_root, jsonl_filename, task_lists):
    ...
    with open(jsonl_filename, 'w') as f:
        for file in sorted(files)[-10:]:
            ...
            # img_dir = Path(data_root) / task / file
            # imgs = sorted(
            #     [str(img_path) for img_path in img_dir.glob('*.png')],
            ...
            imgs = sorted(
                [str(img_path) for img_path in img_dir.glob('*.png') 
                if img_path.stem.isdigit() and int(img_path.stem) <= 3],
                key=lambda x: int(Path(x).stem),
                reverse=True
            )[:-1]
            # imgs = [imgs[i] for i in [0, 3, 6, 9]]

            for img_i in range(len(imgs)-1):
                num_left_word = num2words(6 - 2 * (img_i + 1)).upper()
                episode_data = {
                    'image_old': [imgs[img_i]],
...
def center_crop(img_path, out_path, crop_size=384):
    ...
    # # 计算裁剪区域的左上角和右下角
    left = (w - crop_size) // 2 + 75
    top = (h - crop_size) // 2 + 90
    right = left + crop_size - 180
    bottom = top + crop_size - 240
...
if __name__ == "__main__":
    ...
    jsonl_filename = f'{json_save_root}/lego_small_1000_no1_test.jsonl'
    json_file = f'{json_save_root}/lego_small_1000_no1_test.json'
    ...
    # # ---------------------------- # 
    # img_list = [
    # "/gpfs/0607-cluster/guchenyang/Data/Janus/Raw/outputSmall_0123/01998953-d635-76cf-93c4-32d356fd00a5/3.png"
    # ]
    # i = 0 
    # for img in img_list:
