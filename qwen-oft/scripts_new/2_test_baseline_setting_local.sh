#!/bin/bash
# Baseline 实验对应测试脚本（最基础版本，无 DINOv3）- 本地单 GPU
# 与 2_train_baseline_setting.sh 的 5 组配置一一对应
# 用法: 可单独运行，或由 2_train_baseline_setting.sh 在全部训练完成后自动调用
# 调试: DEBUG=1 [DEBUG_PORT=5678] bash 2_test_baseline_setting_local.sh
# 与其它 local 脚本并行: 使用 DISPLAY_OFFSET 区分 Xvfb（本脚本默认 :2）
...
echo "========================================"
echo "2_test_baseline_setting_local - 本地单 GPU 评测"
echo "========================================"
...
# 与 2_train_baseline_setting.sh 中 5 组 baseline 的 run_name 对应
run_names=(
  # "baseline_bs8_lr1e4_minlr0_res224_aug1"
  # "baseline_bs4_lr5e5_minlr0_res256_aug1"
  # "baseline_bs4_lr1e5_minlr0_res256_aug1"
  # "baseline_bs4_lr1e5_minlr0_res336_aug1"  # 无法跑, 因为336/16=21, 这时候没法进行2倍下采样, 所以base model 256就OK了
  # "baseline_bs4_lr1e5_minlr0_res336_aug0"
)
models=()
for r in "${run_names[@]}"; do
  base_dir="${project_root}/exp/qwen_oft/${r}"
...
# DISPLAY 偏移，与 1_test/3_test local 并行时用不同 display（本脚本默认 :2）
DISPLAY_OFFSET=5
...
# 启动 Xvfb，display = gpu_id + DISPLAY_OFFSET
