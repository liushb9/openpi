#!/bin/bash
# 模块化剪枝超参搜索 - uniform32（DINOv3 每层 -> Qwen 后 32 层）
...
# uniform32 映射（0-based）：
#   - DINOv3 一共 32 层：0~31
#   - Qwen 一共 36 层：0~35
#   - uniform32: DINOv3 每一层：0~31 -> Qwen 的 4~35 共 32 层
UNIFORM32_MAPPING="[(0,4),(1,5),(2,6),(3,7),(4,8),(5,9),(6,10),(7,11),(8,12),(9,13),(10,14),(11,15),(12,16),(13,17),(14,18),(15,19),(16,20),(17,21),(18,22),(19,23),(20,24),(21,25),(22,26),(23,27),(24,28),(25,29),(26,30),(27,31),(28,32),(29,33),(30,34),(31,35)]"
...
DINOV3_QWEN_MAPPING="${UNIFORM32_MAPPING}"
...
# 剪枝参考层搜索（0-based）：
# - 4 层（与之前设置一致）
# - 提前一层：对于 uniform32 对应选 3 层
PRUNING_REF_LAYERS=(1 3)
...
echo "=========================================="
echo "5_train_module_uniform32_4tasks: 剪枝超参数搜索"
echo "  uniform: 32"
echo "  bs: ${BS}"
echo "  lr: ${LR}"
...
for MODE in "${PRUNING_MODES[@]}"; do
  for REF_LAYER in "${PRUNING_REF_LAYERS[@]}"; do
    for RATIO in "${PRUNING_RATIOS[@]}"; do
        ...
        run_name="module_uniform32_res512_${MODE}_${REF_LAYER}_${RATIO}_epoch${ep}_bs${BS}_lr${LR_STR}_minlr${MIN_LR_RATIO}_res${QWEN_RES}_aug${IMAGE_AUG}"
...
echo "=========================================="
echo "5_train_module_uniform32_4tasks: 所有剪枝配置训练完成。"
echo "即将启动对应测试脚本 5_test_module_uniform32_4tasks.sh ..."
bash ${project_path}/scripts_new/5_test_module_uniform32_4tasks.sh
echo "=========================================="
