#!/bin/bash
# Best 4tasks 实验对应测试脚本（本地单 GPU）
# 与 3_train_best_4tasks.sh 的 6 组配置一一对应：1 baseline + 5 DINOv3
# 可单独运行，或由 3_train_best_4tasks.sh 在全部训练完成后自动调用
...
echo "========================================"
echo "3_test_best_4tasks_local - 本地单 GPU 评测"
echo "========================================"

DEBUG=${DEBUG:-0}
DEBUG_PORT=${DEBUG_PORT:-5678}

# 与 3_train_best_4tasks.sh 的 experiment_name 对应，模型保存在 exp/qwen_oft_new_setting_4tasks/
EXP_MODEL="qwen_oft_new_setting_4tasks"
run_names=(
  # "best4_baseline_bs4_lr1e5_minlr0_res256_aug1"
  # "best4_uniform_half_res224_epoch300"
  # "best4_uniform_half_res384_epoch300"
  "best4_uniform_half_res512_epoch300"
  # "best4_uniform_half_res512_qwen_action_4_0.5_epoch300"
  # "best4_uniform_half_res512_qwen_task_4_0.5_epoch300"
)
models=()
...
EXP="qwen_oft_new_setting_4tasks"
...
# DISPLAY 偏移，用于与 1_test_baseline_ablation_local.sh 并行：默认用 :1，避免 RLBench/Xvfb 冲突
DISPLAY_OFFSET=10
...
# display = gpu_id + DISPLAY_OFFSET，与 1_test_baseline_ablation_local 并行时用不同 display
