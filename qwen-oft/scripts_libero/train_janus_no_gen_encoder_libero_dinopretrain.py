...
LIBERO_DATA_DICT = {
    'libero_spatial': '/mnt/cpfs/wuzhuangzhe/qwen-oft/libero/libero_spatial_no_noops_image/libero_spatial.json',
    'libero_spatial_oneshot': '/mnt/cpfs/wuzhuangzhe/qwen-oft/libero/libero_spatial_no_noops_image/libero_spatial_latent_8_interval_1_warmup_1.json',
    'libero_object': '/mnt/cpfs/wuzhuangzhe/qwen-oft/libero/libero_object_no_noops_image/libero_object.json',
    'libero_goal': '/mnt/cpfs/wuzhuangzhe/qwen-oft/libero/libero_goal_no_noops_image/libero_goal.json',
    'libero_10': '/mnt/cpfs/wuzhuangzhe/qwen-oft/libero/libero_10_no_noops_image/libero_10.json',
}
LIBERO_DATA_STATISTICS_DICT = {
    'libero_spatial': '/mnt/cpfs/wuzhuangzhe/qwen-oft/libero/libero_spatial_no_noops_image/libero_spatial_statistics.json',
    'libero_spatial_oneshot': '/mnt/cpfs/wuzhuangzhe/qwen-oft/libero/libero_spatial_no_noops_image/libero_spatial_latent_8_interval_1_warmup_1_statistics.json',
    'libero_object': '/mnt/cpfs/wuzhuangzhe/qwen-oft/libero/libero_object_no_noops_image/libero_object_statistics.json',
    'libero_goal': '/mnt/cpfs/wuzhuangzhe/qwen-oft/libero/libero_goal_no_noops_image/libero_goal_statistics.json',
    'libero_10': '/mnt/cpfs/wuzhuangzhe/qwen-oft/libero/libero_10_no_noops_image/libero_10_statistics.json',
}
...
class SftDataset(Dataset):
    def __init__(self, config, processor, accelerator, model, dinov3_processor=None):
        ...
        data_json_paths = [LIBERO_DATA_DICT[data_name] for data_name in self.config.data_list]
        data_statistics_paths = [LIBERO_DATA_STATISTICS_DICT[data_name] for data_name in self.config.data_list]

        # data_json_paths = [
        #     '/mnt/world_foundational_model/luoyulin_ckpt/qwen-oft/data/libero/libero_spatial_no_noops_image/libero_spatial.json',
        #     '/mnt/world_foundational_model/luoyulin_ckpt/qwen-oft/data/libero/libero_object_no_noops_image/libero_object.json',
        #     '/mnt/world_foundational_model/luoyulin_ckpt/qwen-oft/data/libero/libero_goal_no_noops_image/libero_goal.json',
        #     '/mnt/world_foundational_model/luoyulin_ckpt/qwen-oft/data/libero/libero_10_no_noops_image/libero_10.json',
        # ]
        # data_statistics_paths = [
        #     '/mnt/world_foundational_model/luoyulin_ckpt/qwen-oft/data/libero/libero_spatial_no_noops_image/libero_spatial_statistics.json',
        #     '/mnt/world_foundational_model/luoyulin_ckpt/qwen-oft/data/libero/libero_object_no_noops_image/libero_object_statistics.json',
        #     '/mnt/world_foundational_model/luoyulin_ckpt/qwen-oft/data/libero/libero_goal_no_noops_image/libero_goal_statistics.json',
        #     '/mnt/world_foundational_model/luoyulin_ckpt/qwen-oft/data/libero/libero_10_no_noops_image/libero_10_statistics.json',
        # ]
        for data_json_path, data_statistics_path in zip(data_json_paths, data_statistics_paths):
            ...
            # 每个 statistics 文件格式: {"libero": {"action": {...}, "state": {...}, "num_transitions": N, "num_trajectories": M}}
            dataset_name = next(iter(data_statistics))
            all_stats_list.append(data_statistics[dataset_name])
        ...
        def _average_stats(stats_list, key):
            ...
            out = {}
            for k in ['mean', 'std', 'max', 'min', 'q01', 'q99']:
        ...
        action_min_7 = np.array(self.stats_data[self.dataset_name]['action']['q01'])
        action_max_7 = np.array(self.stats_data[self.dataset_name]['action']['q99'])
    ...
    def collate_fn(self, batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        for x in batch:
            ...
            if type(x['input_image']) is list:
                ...
                x['input_image'] = [path for path in x['input_image'] if 'agentview' in path]
...
def train(args: argparse.Namespace) -> None:
    ...
    # Load checkpoint weights for action_head, dinov3_model, and dinov3_aligner if available
    # Check if model_path contains a checkpoint folder structure
    checkpoint_parent = args.model_path
    
    # Try to detect if model_path points to a checkpoint folder
    # Expected structure: checkpoint-{epoch}-{step}/tfmr or checkpoint-{epoch}-{step}
    if os.path.basename(checkpoint_parent) == 'tfmr':
        checkpoint_parent = os.path.dirname(checkpoint_parent)
    
    if os.path.exists(checkpoint_parent) and os.path.basename(checkpoint_parent).startswith('checkpoint-'):
        accelerator.print(f"\n{'='*60}")
        accelerator.print(f"Detected checkpoint folder: {checkpoint_parent}")
        accelerator.print(f"Attempting to load additional modules...")
        accelerator.print(f"{'='*60}\n")
        
        # Load action_head
        # action_head_path = os.path.join(checkpoint_parent, 'action_head.pt')
        ...
        #     accelerator.print(f"✓ Loaded action_head from {action_head_path}")
        # else:
        #     accelerator.print(f"⚠ action_head.pt not found at {action_head_path}, using random initialization")
        
        # Load dinov3_model (if full finetune was enabled)
        if args.use_dinov3:
            dinov3_model_dir = os.path.join(checkpoint_parent, 'dinov3_model')
            dinov3_model_path = os.path.join(dinov3_model_dir, 'pytorch_model.bin')
            if os.path.exists(dinov3_model_path):
                dinov3_state_dict = torch.load(dinov3_model_path, map_location='cpu')
                model.dinov3_model.load_state_dict(dinov3_state_dict)
                accelerator.print(f"✓ Loaded dinov3_model from {dinov3_model_path}")
            else:
                accelerator.print(f"⚠ dinov3_model not found at {dinov3_model_path}, using original pretrained weights")
            
            # Load dinov3_aligner
            dinov3_aligner_path = os.path.join(checkpoint_parent, 'dinov3_aligner.pt')
            if os.path.exists(dinov3_aligner_path):
                dinov3_aligner_state_dict = torch.load(dinov3_aligner_path, map_location='cpu')
                model.dinov3_aligner.load_state_dict(dinov3_aligner_state_dict)
                accelerator.print(f"✓ Loaded dinov3_aligner from {dinov3_aligner_path}")
            else:
                accelerator.print(f"⚠ dinov3_aligner.pt not found at {dinov3_aligner_path}, using random initialization")
