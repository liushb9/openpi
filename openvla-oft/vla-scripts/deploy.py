...
class OpenVLAServer:
    def __init__(self, cfg) -> Path:
        ...
        # in `finetune.py`), redirect to that subdirectory for VLA loading, but keep the
        # parent directory for processor/action_head loading.
        ...
        if os.path.isdir(ckpt_path) and os.path.isdir(lora_dir):
            # Copy dataset stats to lora_adapter if needed
        ...
        # Load model (uses lora_adapter path)
        self.vla = get_vla(cfg)

        # Switch back to parent dir for action_head/processor loading
    ...
    def get_server_action(self, payload: Dict[str, Any]) -> str:
        try:
            ...
            # JSON 传输会把 numpy array 变成嵌套 list，这里转回 numpy array
            for k in list(obs_for_model.keys()):
                if k == "instruction":
                    continue
                v = obs_for_model[k]
                if isinstance(v, list):
                    obs_for_model[k] = np.asarray(v, dtype=np.uint8)
            ...
            # 将 numpy array / list[ndarray] 转为普通 Python list，避免客户端依赖 json_numpy
            if isinstance(action, np.ndarray):
                action = action.tolist()
            elif isinstance(action, (list, tuple)):
                action = [a.tolist() if isinstance(a, np.ndarray) else a for a in action]

            if double_encode:
                return JSONResponse(json.dumps(action))
            else:
